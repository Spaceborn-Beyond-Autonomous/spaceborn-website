...
function SpotlightCard({ children, spotlightColor, className }: SpotlightCardProps) {
    ...
    return (
        <div className={`relative ${className}`} onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
            {isHovering && (
                <div
    ...
    return (
        <section id="products" className="py-20 relative overflow-hidden bg-black">
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid lg:grid-cols-[0.7fr_1.4fr_1.1fr] gap-12 items-center mb-16">
                    <div className="grid gap-6">
                        <p className='uppercase font-bold text-[1.4rem] text-gray-200'>Top Products</p>
                        {products.map((product, index) => {
                            const ProductIcon = product.icon
                            const isSelected = selectedProduct === index

                            return (
                                <button key={product.name} onClick={() => setSelectedProduct(index)} className="w-full text-left focus:outline-none group">
                                    <SpotlightCard
                                        spotlightColor="rgba(255, 255, 255, 0.1)"
                                        className={`cursor-pointer transition-all duration-300 rounded-xl cartoon-border2 overflow-hidden ${isSelected
                        ...
                        <Button className="bg-black border w-full cursor-pointer hover:bg-white/5 group transition-all duration-200">
                            <span className="uppercase tracking-wider font-semibold">View more</span>
                            <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
                        </Button>
                    </div>
                    <div className="relative h-full rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm overflow-hidden group">
                    ...
                    <div>
                        ...
                        <Button className="bg-white w-full cursor-pointer text-black hover:bg-white/70 group transition-all duration-200">
